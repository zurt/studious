...
The `record_exercise_completion` tool call returned but the model's tool input was missing `answer` or sent an empty/invalid `examples` list. Logged in `llm_audit.YYYY-MM.jsonl` with `job_type=exercise_completion`, `status=error`. There are **two distinct causes** — check `stop_reason` in the audit entry to tell them apart:
