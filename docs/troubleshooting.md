...
### Exercise completion fails with "tool response missing `answer` or `examples`" (or the truncated variant)
The `record_exercise_completion` tool call returned but the model's tool input was missing `answer` or sent an empty/invalid `examples` list. Logged in `llm_audit.YYYY-MM.jsonl` with `job_type=exercise_completion`, `status=error`. The usual cause is **truncation**: this is a large forced tool call (answer + answer_english + explanation + three fully-glossed Japanese examples, each with japanese/reading/english/explanation) run at `xhigh` effort, and the output ran past `max_tokens` before `examples` finished serializing — the SDK then hands back a partial tool input. It was intermittent because short drill items fit the budget and long ones didn't. The request budget is now `max_tokens: 8192` (was 2048; `backend/app/api/regions.py`), matching breakdown. If `stop_reason=max_tokens`, the error message says the response was truncated explicitly — retry, or split/simplify the item. If instead the model set `no_exercise: true`, the job fails with a `no_exercise:` message (that's the model correctly reporting the target line isn't a drill item, audited as success). The operation is idempotent and overwrites stale state when `overwrite=true`.
